...
{
    public class RoleRepositories : SimpleCrudRepository<RoleDto, int>, IRole
    {
        private readonly SoDauBaiContext _context;

        public RoleRepositories(SoDauBaiContext context, IDbConnection connection)
       : base(connection)
        {
        ...
        public async Task<(IEnumerable<RoleDto>, int total)> GetRoles(PaginationRequest pagination)
        {
            try
            {
                var sql = """
                        ...
                        using (var reader = ExcelReaderFactory.CreateReader(stream))
                        {
                            ...
                            do
                            {
                                while (reader.Read())
                                {
                                    ...
                                    var myRole = new Domain.Entities.Role
                                    {
